...
def gps_to_h3(
    ...
    """
    if boundary is None:
        return [h3.latlng_to_cell(lat, lon, resolution) for lon, lat in gps_points]
...
def preprocess_resolution(
    ...
    output_dir: str = '/local/data1/shared_data/mixed_res',
    save_csv: bool = False,
    ...
    data_path = {
        'geolife':'/local/data1/shared_data/raw_aggrigated/geolife_aggregated.csv',
        'rome': '/local/data1/shared_data/raw_aggrigated/rome_taxi_aggregated.csv',
        'porto': '/local/data1/shared_data/raw_aggrigated/porto.csv'
    }

    file_path = data_path[dataset]
    print(f"Processing file: {file_path}")

    target_column = 'route_points' if 'geolife' in file_path or 'rome_taxi' in file_path else 'POLYLINE'
    df = pd.read_csv(file_path, usecols=[target_column]).rename(columns={target_column: 'points'})
...
def mixed_resolution(
    ...
    min_resolution: int = 7,
    max_resolution: int = 9,
    input_dir: str = '/local/data1/shared_data/mixed_res',
    output_dir: str = '/local/data1/shared_data/mixed_res',
    ...
    visualize({hexagon: h3.get_resolution(hexagon) for hexagon in hexagon_set}, "/home/anadiri/Trajectory-prediction/mixed-res-heatmap.pdf", categorical=True, cmap=ListedColormap(["#66CDAA", "#9370DB", "#86DAE3"]), legend_kwds={"loc": "upper right", "title":"Resolution", "markerscale":2.5},)
    visualize({hexagon: hex_counts[hexagon] for hexagon in hexes[min_resolution]}, "/home/anadiri/Trajectory-prediction/mixed-res-map.pdf", bins=[0,500,10000,np.inf], cmap=ListedColormap(["#66CDAA", "#9370DB", "#86DAE3"]), categorical=True, k=10, legend_kwds={"loc": "upper right", "title":"Movement Density", "markerscale":2.5,},)
    print("Heatmap saved")


if __name__ == "__main__":
    # preprocess_resolution(dataset='geolife')
    mixed_resolution(
